...
namespace BWTA {
  ...
  void regionColoring() {
    ...
    color.resize(numRegions); // stores color of the vertices (0 if not colored it yet)
    std::vector<size_t> NN;
    NN.resize(numRegions); // stores all the vertices that is not adjacent to current vertex
  ...
  void regionColoringHUE() {
    size_t numRegions = BWTA_Result::regions.size();
    std::vector<double> hueList;
    hueList.resize(numRegions);
