...
namespace BWEM {
  ...
  void Area::AddGeyser(Geyser* pGeyser) {
    bwem_assert(pGeyser && not contains(m_Geysers, pGeyser));
    m_Geysers.push_back(pGeyser);
  }


  void Area::AddMineral(Mineral* pMineral) {
    bwem_assert(pMineral && not contains(m_Minerals, pMineral));
  ...
  void Area::CreateBases() {
    ...
    for (Mineral* m : Minerals()) if ((m->InitialAmount() >= 40) && not m->Blocking()) RemainingRessources.push_back(m);
    for (Geyser* g : Geysers()) if ((g->InitialAmount() >= 300) && not g->Blocking()) RemainingRessources.push_back(g);

    m_Bases.reserve(min(100, (int)RemainingRessources.size()));

    while (not RemainingRessources.empty()) {
