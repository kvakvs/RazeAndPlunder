...
namespace BWEM {
  namespace detail {
    ...
    void Graph::CreateChokePoints() {
      ...
      for (auto& raw : RawFrontierByAreaPair) {
        ...
        for (auto w : RawFrontierAB) {
          ...
          if (not added) Clusters.push_back(deque<WalkPosition>(1, w));
        }
      ...
      for (Neutral* pNeutral : BlockingNeutrals)
        if (not pNeutral->NextStacked()) // in the case where several neutrals are stacked, we only consider the top
        {
      ...
      for (Area::id a = 1; a <= AreasCount(); ++a)
        for (Area::id b = 1; b < a; ++b)
          if (not GetChokePoints(a, b).empty()) {
    ...
    const CPPath& Graph::GetPath(const Position& a, const Position& b, int* pLength) const {
      ...
      if (not pAreaA->AccessibleFrom(pAreaB)) {
      ...
      for (const ChokePoint* cpA : pAreaA->ChokePoints())
        if (not cpA->Blocked()) {
          const int dist_A_cpA = a.getApproxDistance(Position(cpA->Center()));
          for (const ChokePoint* cpB : pAreaB->ChokePoints())
            if (not cpB->Blocked()) {
  ...
  Graph::Graph(MapImpl* pMap)
      : m_pMap(pMap), m_Areas(), m_ChokePointList(), m_ChokePointsMatrix()
      , m_PathsBetweenChokePoints(), m_EmptyPath()
  {
  }
